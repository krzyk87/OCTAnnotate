...
void QCPGraph::getPlotData(QVector<QPointF> *lineData, QVector<QCPData> *scatterData) const
{
  switch(mLineStyle)
  {
    ...
    case lsDiscontinuousLine: getDiscontinuousLinePlotData(lineData, scatterData); break;
  }
}
...
void QCPGraph::getDiscontinuousLinePlotData(QVector<QPointF> *linePixelData, QVector<QCPData> *scatterData) const
{
    QCPAxis *keyAxis = mKeyAxis.data();
    QCPAxis *valueAxis = mValueAxis.data();
    if (!keyAxis || !valueAxis) { qDebug() << Q_FUNC_INFO << "invalid key or value axis"; return; }
    ...
    //Need to figure out a better way to handle this....fixed at 90 seconds for now
    int lineCount = 0;
    ...
    if (keyAxis->orientation() == Qt::Vertical)
    {
      for (int i=1; i<lineData.size(); ++i)
      {
        if( lineData.at(i).pointsContinuous )
        {
          (*linePixelData)[lineCount*2+0].setX(valueAxis->coordToPixel(lineData.at(i-1).value));
          (*linePixelData)[lineCount*2+0].setY(keyAxis->coordToPixel(lineData.at(i-1).key));
          (*linePixelData)[lineCount*2+1].setX(valueAxis->coordToPixel(lineData.at(i).value));
          (*linePixelData)[lineCount*2+1].setY(keyAxis->coordToPixel(lineData.at(i).key));
          lineCount++;
        }
      }
    } else // key axis is horizontal
    {
      for (int i=1; i<lineData.size(); ++i)
      {
        if( lineData.at(i).pointsContinuous )
        {
          (*linePixelData)[lineCount*2+0].setX(keyAxis->coordToPixel(lineData.at(i-1).key));
          (*linePixelData)[lineCount*2+0].setY(valueAxis->coordToPixel(lineData.at(i-1).value));
          (*linePixelData)[lineCount*2+1].setX(keyAxis->coordToPixel(lineData.at(i).key));
          (*linePixelData)[lineCount*2+1].setY(valueAxis->coordToPixel(lineData.at(i).value));
          lineCount++;
        }
      }
    }
    ...
    linePixelData->resize(lineCount*2);
}
...
//  //Need to figure out a better way to handle this....fixed at 90 seconds for now
//  double dataGapInterval = 90;  //BRG
//  bool pointsContinuous = true;
...
//            {
//              lineData->append(QCPData(currentIntervalStartKey+keyEpsilon*0.2, currentIntervalFirstPoint.value().value));
//              lineData->last().pointsContinuous = pointsContinuous;
//            }
//            lineData->append(QCPData(currentIntervalStartKey+keyEpsilon*0.25, minValue));
//            lineData->last().pointsContinuous = pointsContinuous;
//            lineData->append(QCPData(currentIntervalStartKey+keyEpsilon*0.75, maxValue));
...
//          pointsContinuous = true;
//        }

//        if( (it.value().key - (it-1).value().key) > dataGapInterval ) //BRG
//            pointsContinuous = false;                                 //BRG
...
//      {
//        dataVector->append(it.value());
//        dataVector->last().pointsContinuous = ((it.value().key - (it-1).value().key) > dataGapInterval ) ? false : true; //BRG
...
int QCustomPlot::heightForWidth(int width) const
{
    int height = width + 50;
    return height;
}
